...
const API_SETTINGS_STORAGE_KEY = "radiology-api-settings";

const defaultApiSettings: ApiSettings = {
  backendBaseUrl: API_BASE_URL,
  medgemmaApiUrl: MEDGEMMA_API_URL,
  medgemmaApiKey: "",
  medgemmaModel: MEDGEMMA_MODEL,
};
...
function loadApiSettings(): ApiSettings {
  try {
    ...
    return { ...defaultApiSettings, ...(JSON.parse(saved) as Partial<ApiSettings>) };
  } catch {
    return defaultApiSettings;
  }
}
...
function medgemmaHeaders(settings: ApiSettings) {
  const headers: Record<string, string> = { "Content-Type": "application/json" };
  if (settings.medgemmaApiKey.trim()) {
    headers.Authorization = `Bearer ${settings.medgemmaApiKey.trim()}`;
  }
  return headers;
}

async function postStudyFile(file: File, settings: ApiSettings) {
  const formData = new FormData();
  formData.append("file", file);

  const response = await fetch(`${normalizeBaseUrl(settings.backendBaseUrl)}/studies`, {
    method: "POST",
    body: formData,
  });
...
async function requestSegmentation(studyId: string, prompt: RoiBox, settings: ApiSettings) {
  const response = await fetch(`${normalizeBaseUrl(settings.backendBaseUrl)}/studies/${studyId}/segmentations/medgemma`, {
    method: "POST",
...
async function requestReport(study: Study, settings: ApiSettings) {
  const segSummary =
...
Respond in this exact JSON format:
{
  ...
  const response = await fetch(`${normalizeBaseUrl(settings.medgemmaApiUrl)}/chat/completions`, {
    method: "POST",
    headers: medgemmaHeaders(settings),
    body: JSON.stringify({
      model: settings.medgemmaModel.trim() || MEDGEMMA_MODEL,
      messages,
      max_tokens: 1024,
      temperature: 0.3,
    }),
...
async function requestLlmStatus(settings: ApiSettings) {
  try {
    const response = await fetch(`${normalizeBaseUrl(settings.medgemmaApiUrl)}/models`, {
      headers: settings.medgemmaApiKey.trim() ? { Authorization: `Bearer ${settings.medgemmaApiKey.trim()}` } : undefined,
    });
    if (!response.ok) {
      throw new Error();
    }
    ...
    const configuredModel = settings.medgemmaModel.trim();
    const matchingModel =
      modelIds.find((id) => id === configuredModel) ??
      modelIds.find((id) => id.toLowerCase().includes("medgemma"));
    const medgemmaAvailable = Boolean(matchingModel);
    ...
    throw new Error("Medgemma API is not reachable at " + settings.medgemmaApiUrl);
  }
}
...
function App() {
  ...
  useBrowserStatus(apiSettings, setLlmStatus);
  ...
  const handleStudyFile = async (file: File) => {
    ...
    try {
      const uploaded = await postStudyFile(file, apiSettings);
      const apiPreviewUrl = uploaded.previewUrl?.startsWith("/") ? `${normalizeBaseUrl(apiSettings.backendBaseUrl)}${uploaded.previewUrl}` : uploaded.previewUrl;
  ...
  const runSegmentation = async (prompt: RoiBox = { x: 0.47, y: 0.34, width: 0.16, height: 0.22 }) => {
    ...
    try {
      const segmentation = await requestSegmentation(study.id, prompt, apiSettings);
  ...
  const runReport = async () => {
    ...
    try {
      const report = await requestReport(study, apiSettings);
      ...
      updateStudy((currentStudy) => ({
        ...
        report: {
          ...
          recommendation: llmStatus.medgemma_configured
            ...
            : `Ensure Medgemma is running at ${apiSettings.medgemmaApiUrl}, then generate the report again.`,
        ...
        <ApiSettingsModal
          ...
          onSave={(nextSettings) => {
            const normalizedSettings = {
              backendBaseUrl: normalizeBaseUrl(nextSettings.backendBaseUrl) || defaultApiSettings.backendBaseUrl,
              medgemmaApiUrl: normalizeBaseUrl(nextSettings.medgemmaApiUrl) || defaultApiSettings.medgemmaApiUrl,
              medgemmaApiKey: nextSettings.medgemmaApiKey.trim(),
              medgemmaModel: nextSettings.medgemmaModel.trim() || defaultApiSettings.medgemmaModel,
            };
            saveApiSettings(normalizedSettings);
...
function useBrowserStatus(settings: ApiSettings, setLlmStatus: (status: LlmStatus) => void) {
  useEffect(() => {
    let cancelled = false;
    ...
    <aside className="flex min-h-0 flex-col border-l border-slate-800 bg-[#080d15] max-lg:min-h-[640px]">
      <header className="flex shrink-0 items-center justify-between border-b border-slate-800 bg-[#090f18] px-4 py-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 truncate text-xs text-slate-500">Model endpoint and report controls</p>
        </div>
        <button
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
        ...
        >
          <p className={`mb-3 text-xs font-medium ${llmStatus.medgemma_configured ? "text-emerald-300" : "text-amber-300"}`}>
            {llmStatus.medgemma_configured ? `Backend model ready: ${llmStatus.model}` : "Backend is in mock mode. Check API settings and confirm the model server is running."}
          </p>
...
function ApiSettingsModal({ settings, onClose, onSave }: ApiSettingsModalProps) {
  ...
  const updateDraft = (key: keyof ApiSettings, value: string) => {
      ...
      <section className="w-full max-w-xl rounded-md border border-cyan-300/30 bg-[#0c1420] shadow-panel-soft">
        <header className="flex items-start justify-between gap-4 border-b border-slate-800 px-5 py-4">
          <div className="min-w-0">
            ...
            <p className="mt-1 truncate text-xs text-slate-500">Runtime configuration is saved in this browser.</p>
          </div>
          <button
        ...
        <div className="space-y-4 p-5">
          <SettingsField
            ...
            onChange={(value) => updateDraft("backendBaseUrl", value)}
          />
          <SettingsField
            label="Model Endpoint URL"
            value={draftSettings.medgemmaApiUrl}
            placeholder="http://127.0.0.1:1234/v1"
            onChange={(value) => updateDraft("medgemmaApiUrl", value)}
          />
          <SettingsField
            icon={KeyRound}
            label="API Key"
            value={draftSettings.medgemmaApiKey}
            placeholder="Optional bearer token"
            type="password"
            onChange={(value) => updateDraft("medgemmaApiKey", value)}
          />
          <SettingsField
            label="Model Identifier"
            value={draftSettings.medgemmaModel}
            placeholder="medgemma-1.5-4b-it"
            onChange={(value) => updateDraft("medgemmaModel", value)}
          />
